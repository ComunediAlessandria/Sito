...
_.indexTreeEditorManager = function(tk, t, opt) {
	...
	if (this.opt.ctrlSave)
		$(this.opt.ctrlSave).click(fjs.proxy(this.onCtrlSave, this));

	if (this.opt.ctrlCancel)
		$(this.opt.ctrlCancel).click(fjs.proxy(this.onCtrlCancel, this));
	...
	var ppb = fjs.proxy(this.onPopButton, this);
/*
	this.pop = new fjs.ui.bst.popover({
...
*/
	...
	this.pop = new fjs.ui.bst.popover({
		...
		title: 'Titolo',
		content: 'Contenuto' + '<div class="pageinfo"></div>',
		footer: ' <button class="btn btn-mini edit" data-op="edit"><i class="icon-edit"></i>&nbsp;' + __('ite.mod') /* Modifica */ + '</button>'
			+ ' <button class="btn btn-mini del" data-op="del"><i class="icon-trash"></i>&nbsp;' + __('ite.del') /* Elimina */ + '</button>'
			+ ' <button class="btn btn-mini add" data-op="add"><i class="icon-plus"></i>&nbsp;' + __('ite.add') /* Aggiungi Nodo */ + '</button>'
			+ ' <button class="btn btn-mini pedit" data-op="pedit"><i class="icon-pencil"></i>&nbsp;' + __('ite.edit') /* Modifica Pagina */ + '</button>'
	});
...
_.indexTreeEditorManager.prototype.popOpen = function(el, p, cb) {
...
_.indexTreeEditorManager.prototype.saveChanges = function(cb) {
	...
	var b = new fjs.ui.notificationBubble({
...
_.indexTreeEditor.prototype.attach = function() {
	...
	this.el.dynatree({
		...
		onLazyRead: fjs.proxy(this.onLazyRead, this),
		onClick: fjs.proxy(this.onNodeClick, this),
...
//				console.log("tree.onDragLeave(%o, %o)", node, sourceNode);
	...
	this.chooser = new fjs.ui.pageChooser({
...
//		text: [ '<div style="text-align: center;">Loading ...</div>'],
		...
		onPageSelected: fjs.proxy(this.onPageSelectedInChooser, this),
	...
	if (this.opt.ctrlExpand) {
		...
		$(this.opt.ctrlExpand).click(fjs.proxy(this.onCtrlExpand, this));
		this.ctrlEls.add(this.opt.ctrlExpand);
	}

	if (this.opt.ctrlAdd) {

		$(this.opt.ctrlAdd).click(fjs.proxy(this.onCtrlAdd, this));
...
//		p = span.position()
	...
	this.manager.pop.$('button.pedit')[ node.data.entity.substr(0, 5) === 'BLOB:' ? 'show' : 'hide' ]();
...
//				content: 'Contenuto'
			...
			fjs.proxy(me.onPopoverButton, me)
		);
...
_.indexTreeEditor.prototype.confirmOperation = function(title, text, cb) {
	...
	var
		me = this,
		d = new fjs.ui.modalDialogWithButtons({
...
_.indexTreeEditor.prototype.addNode = function(node) {
	...
	var
		me = this,
		b = new fjs.ui.notificationBubble({
