...
try {
	...
	// ci sarà solo 1 record, ma gestico comunque l'ID
	
	$recs = i18nQuaria()->getRecordList('MisureAntismog', sql::where('1>0'));
	...
	elseif ($n > 1)
		throw new Exception("Troppi record antismog: verificare!");
	else {

		$recs = reset($recs);

		$id = $recs['IDMisureAntismog'];

		$data = array(
			'DataDa'	=> dateUtils::Myslq2TS($recs['DataDa'], false),
			'DataA'		=> dateUtils::Myslq2TS($recs['DataA'], false),	
			'Livello'	=> $recs['Livello'],
			'Note'		=> $recs['Note'],
		);
	}
